...
// ═══════════════════════════════════════════════════════════
// Component catalog — maps component chars to labels (en/es)
// Loaded from data/component_catalog.json
// ═══════════════════════════════════════════════════════════
...
function getLabel(char: string): string {
  const entry = COMPONENT_LABELS[char];
  if (entry) return entry.es;
  return char;
}

function getLabelEn(char: string): string {
  const entry = COMPONENT_LABELS[char];
...
const CONFUSABLES: Record<string, string[]> = {
  // ── Component/radical confusables (existing) ──────
  '亻': ['彳', '人', '入'],
  '氵': ['シ', '冫', '水'],
  '扌': ['才', '手', '寸'],
  '糸': ['系', '紙', '線'],
  ...
  // Additional remaining single kanji
  '欠': ['次', '飲', '歌', '吹'],
  '書': ['画', '事'],
  '事': ['書', '筆'],
  '色': ['魚', '角'],
...
function main(): void {
    ...
    if (raw && raw.parts.length >= 2) {
      decompOut[k] = {
        ...
        components: raw.parts.map((p, i) => {
          if (!COMPONENT_LABELS[p.char]) missingLabels.push(p.char);
          return {
            id: `${k}_c${i}`,
            char: p.char,
            label: getLabel(p.char),
            label_en: getLabelEn(p.char),
            slot_id: p.slot,
            order_index: i,
      ...
      decompOut[k] = {
        layout: 'single',
        components: [
          { id: `${k}_c0`, char: k, label: getLabel(k), label_en: getLabelEn(k),
            slot_id: 'center', order_index: 0,
  ...
  // Report components missing from the catalog
  const uniqueMissingLabels = [...new Set(missingLabels)];
  if (uniqueMissingLabels.length > 0) {
    console.warn(`\n⚠  ${uniqueMissingLabels.length} components missing from catalog (using char as label):`);
    console.warn(`   ${uniqueMissingLabels.join(' ')}`);
  }
  ...
  for (const [text, def] of Object.entries(WORD_DICTIONARY)) {
    // check every kanji in the word has a decomposition (single or multi)
    const allKnown = def.kanji.every((k) => allKanjiSet.has(k));
    if (allKnown) {
      wordsOut.push({ id: `w${wid++}`, ...def });
    }
  }
  console.log(`Words: ${wordsOut.length} playable`);
